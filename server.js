 ...
 * - WS /ws: browser audio <-> Gemini Bidi (TEXT only)
 * - Enforces LMS key + student course scope
 * - Supports tool calling (Google Web + YouTube search)
 *
 * NOTE: Gemini returns TEXT only. Frontend handles TTS via SpeechSynthesis.
 */
...
// Serve frontend from /public (voice-app.html, voice-app.js, voice-app.css)
...
// -----------------------------------------------------------------------------
// Gemini Bidi API bridge (TEXT ONLY, model = gemini-2.0-flash)
// -----------------------------------------------------------------------------
const GEMINI_LIVE_WS =
  "wss://generativelanguage.googleapis.com/ws/google.ai.generativelanguage.v1beta.GenerativeService.BidiGenerateContent";

// Only certain models support Bidi. gemini-2.0-flash does.
// Default to that; allow override with GEMINI_LIVE_MODEL.
const LIVE_MODEL =
...
3) Prefer authoritative sources when searching the web (including Google Search and YouTube tools).
4) Do NOT invent links.
`;

function openGeminiLiveSocket({ systemInstruction }) {
    ...
    const setupPayload = {
      setup: {
        ...
        generationConfig: {
          temperature: 0.4,
          maxOutputTokens: 512,
          // ⚠ no responseModalities, no speechConfig → avoid 1007
        },
        systemInstruction: {
          parts: [{ text: systemInstruction }],
        },
        tools, // tool object defined above
      },
    };
...
wss.on("connection", (clientWs) => {
  ...
  clientWs.on("message", async (raw) => {
    ...
    if (msg.type === "start") {
      try {
        ...
        geminiWs.on("message", async (data) => {
          ...
          // 🔍 DEBUG entire server message (first few only)
          // console.log("🔁 Gemini message:", JSON.stringify(gm, null, 2));
          ...
          const modelTurn = gm?.serverContent?.modelTurn;
          const parts = modelTurn?.parts || [];

          for (const p of parts) {
            if (p.text) {
              // Send transcript chunk to frontend
              sendClient({ type: "text", text: p.text });
            }
            // We no longer expect inlineData audio here; TTS is in browser
          }
    ...
    // Audio from browser → Gemini (for STT)
    if (msg.type === "audio") {
      geminiWs.send(
        JSON.stringify({
          realtimeInput: {
